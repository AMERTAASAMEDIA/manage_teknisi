<template>
  <CRow>
    <CCol :xs="12">
      <CCard class="mb-4">
        <CCardHeader>
          <strong>Laporan Pemakaian Alat(Bulanan)</strong> <small>#</small>
        </CCardHeader>
        <CCardBody>
          <CRow class="mb-6">
          ...
          <CTable bordered>
            <CTableHead>
              <CTableRow>
                ...
                <CTableHeaderCell scope="col">Nama Item</CTableHeaderCell>
                <CTableHeaderCell scope="col">Kode</CTableHeaderCell>
                <CTableHeaderCell scope="col">PCS</CTableHeaderCell>
            ...
            <CTableBody>
              <CTableRow v-for="(items, index) in FilterPost" :key="items._id">
                ...
                <CTableDataCell>{{ items.Tr_logistic_keluar_nama_item }}</CTableDataCell>
                <!-- <CTableDataCell>
                                        <CButton color="secondary" role="button" @click="lihatgambar(items)">Lihat</CButton>
                                    </CTableDataCell> -->
                <CTableDataCell>{{ items.Tr_logistic_keluar_kode_transaksi }}</CTableDataCell>
                <CTableDataCell>{{ items.Tr_logistic_keluar_total }}</CTableDataCell>
                <CTableDataCell>{{ items.Tr_logistic_keluar_created }}</CTableDataCell>
...
<script src="./Service/laporan_pemakaian_alat.jsx"></script>
