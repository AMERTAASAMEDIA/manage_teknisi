<template>
  <CRow>
    ...
    <CCol :xs="12">
      <CCard class="mb-4">
        <CCardHeader> <strong>Data Item</strong> <small>#</small> </CCardHeader>
        <CCardBody>
          <CRow class="mb-6">
          ...
          <CTable bordered>
            ...
            <CTableBody>
              <CTableRow v-for="(items, index) in FilterPost" :key="items._id">
                ...
                <CTableDataCell v-if="items.item_list_status_pemakaian === 'N'"
                  >Belum Dipakai
                </CTableDataCell>
                <CTableDataCell v-if="items.item_list_status_pemakaian === 'Y'"
                  >Dipakai</CTableDataCell
                >
                <CTableDataCell>{{
                  formatCurrency(parseFloat(items.item_list_harga))
                }}</CTableDataCell>
                <CTableDataCell>
                  <CDropdown>
                    ...
                    <CDropdownMenu>
                      ...
                      <CDropdownItem
                        ><router-link
                          ...
                          :to="{ name: 'HistoryPemakaianList', params: { id: items._id } }"
                          >History Pemakaian</router-link
                        >
                      </CDropdownItem>
                      <CDropdownItem v-if="items.item_list_status_pemakaian === 'N'"
                        ><router-link
                          style="text-decoration: none"
                          :to="{ name: 'AlokasiBarang', params: { id: items._id } }"
                          >Transaksi</router-link
                        >
                      </CDropdownItem>
                      <CDropdownItem v-if="items.item_list_status_pemakaian === 'Y'">
                        <a color="primary" role="button" @click="reinput(items._id)"
                          >Re-Input barang
                        </a>
                      </CDropdownItem>
                      <CDropdownItem
                        ><router-link
                          style="text-decoration: none"
                          :to="{ name: 'HistoryPembelian', params: { id: items._id } }"
                          >Tahap QC</router-link
                        >
                      </CDropdownItem>
                    </CDropdownMenu>
                  </CDropdown>
          ...
          <CPagination aria-label="Page navigation example">
            <CPaginationItem @click="prev()" aria-label="Previous" href="#"
            ...
            <CPaginationItem @click="next()" aria-label="Next" href="#"
...
<script src="./Service/data_item_pcs.jsx"></script>
